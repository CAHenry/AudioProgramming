...
DSP_buffer* DSP_createBuffer (unsigned int bufferSize)
{
    ...
    {
        DSP_errorHandler (dsp_war_buffer_too_big, maxBufferSize);
        bufferSize = maxBufferSize;
    }
    else if (DSP_isPowerOfTwo (bufferSize) == false)
    {
        DSP_errorHandler (dsp_war_buffer_not_pow_two, defaultBufferSize);
    ...
    buffer->data = alloca (sizeof (double) * buffer->size);
    DSP_clearBuffer (buffer);
    return buffer;
}

void DSP_destroyBuffer (DSP_buffer* buffer)
{
    free(buffer);
}
